    ...
    for hour in range(0, 24):
        ...
        for path in paths:
            ...
            with nc.Dataset(path) as ds:
                if ds['Precipitation'].shape[0] < 1000:
                    print('Rows and cols of {} are too small'.format(os.path.basename(path)))
                    continue
            # 得到数据块，并存储为nc
            try:
                fy4a.get_data_from_data_name(path, precipitation_name, new_lon, new_lat, sv_path=sv_path)
            except:
                print('{} can not open'.format(os.path.basename(path)))
                continue
        ...
        for path in paths:
            with nc.Dataset(path) as ds:
                ...
                lat = ds['lat'][:]
            os.remove(path)
        if len(dataset_hours) == 0:
            print('Time: {}{:02d} is not exist, maybe the rows and cols, or error about opening.'.format(date_str, hour))
            continue
